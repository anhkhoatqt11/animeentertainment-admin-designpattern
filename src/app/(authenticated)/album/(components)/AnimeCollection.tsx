...
function AnimeCollection({ animeAlbumList, setAnimeAlbumList }) {
  const { isOpen, onOpen, onOpenChange } = useDisclosure();
  const [subjectId, setSubjectId] = useState();
  const { deleteAnimeAlbum } = useAlbum();
  const itemClasses = {
    base: "py-0 w-full",
  ...
  const onDeleteAlbum = async () => {
    setAnimeAlbumList(animeAlbumList.filter((i) => i._id !== subjectId));
    await deleteAnimeAlbum({ _id: subjectId }).then((res) => {
      ...
      <div className="grid-cols-1 grid gap-4 mb-6">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:justify-between items-end p-4 pb-0">
          ...
          <Button
            ...
            onClick={() => {
              router.push("/album/anime/addNewAlbum");
            }}
          >
              ...
              >
                <div>
                  <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
                    {item?.list.map((item) => (
                      <AnimeItem item={item} key={`anime-${item._id}`} />
                    ))}
                  </div>
                  <div className="grid grid-cols-2 gap-3 mt-3">
                    <Button
                      ...
                      onClick={() => {
                        router.push(`album/anime/${item._id}`);
                      }}
                    >
                      Sửa album
                    </Button>
